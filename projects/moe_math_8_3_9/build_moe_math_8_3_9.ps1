param(
    ...
    [string]$SourceUrl = "https://priori.moe.gov.tw/upload/page/resource/material/math/grade8/book3/math-8-3-9.pdf"
)

$ErrorActionPreference = "Stop"

$ProjectDir = Join-Path $RepoDir "projects\moe_math_8_3_9"
$OutputDir = Join-Path $ProjectDir "output"
$TmpDir = Join-Path $ProjectDir "tmp"
$SegmentsPath = Join-Path $ProjectDir "segments.json"
$ScenePath = Join-Path $ProjectDir "moe_math_8_3_9_scene.py"
...
$commandParts = @(
    "powershell -ExecutionPolicy Bypass -File `"$ProjectDir\build_moe_math_8_3_9.ps1`"",
    "-TtsEngine $TtsEngine",
    "-VoiceName `"$VoiceName`"",
    "-EdgeRate `"$EdgeRate`"",
...
$audioPath = Join-Path $OutputDir "moe_math_8_3_9_narration.wav"
...
$animationStem = "moe_math_8_3_9_animation" + $variantSuffix
$renderArgs = @($ScenePath, "FactoringQuadraticTeaching", "-w", "-r", "1280x720", "--fps", "30", "--video_dir", $videoDir, "--file_name", $animationStem, "--quiet")
if (-not [string]::IsNullOrWhiteSpace($BackgroundColor)) {
...
$narratedVideo = Join-Path $OutputDir ("moe_math_8_3_9_narrated" + $variantSuffix + ".mp4")
$finalName = "moe_math_8_3_9_stevenke1981" + $variantSuffix + "_subtitled.mp4"
...
$log = @(
    "# Factoring quadratic equation teaching video process log",
    "",
    "- Skill used: manimgl-teaching-video.",
    "- Skill used: manimgl-3b1b for ManimGL rendering.",
    "- Skill used: pdf for PDF extraction and page preview.",
    "- Skill used: ffmpeg for audio/video/subtitle assembly.",
    "- Reference PDF URL: $SourceUrl.",
    "- Source note: the video uses an original script and original ManimGL visuals; source branding is not displayed in the video.",
    "- On-screen production credit: stevenke1981 production credit.",
